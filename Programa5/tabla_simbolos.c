...
/*
* Función: set_sym
* Descripción: Llena todos los datos de un simbolo
* Autor: Osmar Juarez Aguilar
* Fecha de creación: 15 de junio 2020
*/
SYM *set_sym(SYM *simbol, char *id, int direccion, int tipo, char *tipoVariable, ARGS *args, SYMTAB *tabSimbolos, TYPTAB *tabTipos){
  strcpy(simbol->id,id);
  simbol->dir = direccion;
  simbol->tipo = tipo;
  tabSimbolos->tt_asoc = tabTipos;
...
/*
* Función: getVar
* Descripción: Retorna el tipo de variable, en caso de no encontrarla regresa -1
* Autor: OJA
* Fecha de creación: 30 de mayo 2020
*/
char *getVar(SYMTAB *st,char *id){
    SYM *simbol = search_SYM(st,id);
  return simbol ? simbol->var : "";
}
