...
%type<valorTipo> tipo base tipo_arreglo
...
%type<valorTipo> arg tipo_registro
%type<valorTipo> tipo_arg param_arr
...
declaraciones:    tipo {typeGBL=$1;} lista_var PYC{} declaraciones
                | tipo_registro {typeGBL=$1;} lista_var PYC{
...
tipo_arreglo:	LCOR NUM RCOR tipo_arreglo {
					...
					if($2.tipe=0){
						...
						if(n>0){
							newTYP = init_type();
							newTYP = set_typ(newTYP,"array",$4,n*getTam(getTopType(pTipos),$4),getTopType(pTipos));
							$$ = append_type(getTopType(pTipos), newTYP);
						}
						else{
...
funciones:        DEF tipo ID {if(search_id_symbol(TGS,$3.lexval) == -1){
									simbol = init_sym();
									simbol = set_sym(simbol,$3.lexval,-1,$2,"func",NULL,TGS,TGT);
...
tipo_arg:         base param_arr {baseGBL = $1;};

param_arr:      LCOR RCOR param_arr{
					TYP *tp = init_type();
					//newTYP = set_typ(init_type(),"array",$4,n*getTam(getTopType(pTipos),$4),getTopType(pTipos));
                    tp = set_typ(tp,"array",$3,0,getTopType(pTipos));
                    $$ = append_type(getTopType(pTipos), tp);
				} | {$$=baseGBL;};
